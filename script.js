...
magicDiv.classList.add("magicClass")
const p = document.createElement("p");
p.textContent = "Плаваюча кнопка"
const closeBtn = document.createElement("button");
magicDiv.append(p, closeBtn)
...
function visuallyAlert(textAlert) {
  magicDiv.classList.remove("magicClass");
  p.textContent = textAlert;
  setTimeout(() => {
    magicDiv.classList.add("magicClass");
  }, 5000)
}
...
function editBook (book) {
  ...
  refSave.addEventListener("click", (event)=> {
    ...
    if(newBooks.title === "" || newBooks.author === "" || newBooks.plot === ""
    || newBooks.img === "") {
      return alert("Заповнені не всі поля")
    }
    const books = JSON.parse(localStorage.getItem("books"));
    const index = books.findIndex(elem => elem.id === idBook);
...
function deleteBook(book) {
  ...
  visuallyAlert("Успішно видалено");
}
...
function addBook() {
  ...
  refSave.addEventListener("click", (evt) => {
    evt.preventDefault();
    if(newBook.title === "" || newBook.author === "" || newBook.plot === ""
    || newBook.img === "") {
      return alert("Заповнені не всі поля")
    }
    const books = JSON.parse(localStorage.getItem("books"));
